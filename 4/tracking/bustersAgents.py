    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghostEstimatedLocations = []
        for dist in livingGhostPositionDistributions:
            maxPos, maxProb = None, -math.inf
            #print(dist)
            for pos in dist:
                #print(pos, prob)
                if dist[pos] > maxProb:
                    ...
                    maxProb = dist[pos]
            if maxProb != -math.inf:
                ghostEstimatedLocations.append(maxPos)
        #print(len(livingGhostPositionDistributions))
        minDistance = math.inf
        ClosestGhost = None
        for ghost in ghostEstimatedLocations:
        ...
        for action in legal:
            ...
            distance = self.distancer.getDistance(successorPos, ClosestGhost)
