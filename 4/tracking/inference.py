...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(PAC, OBS0), (PAC, OBS1),
             (GHOST0, OBS0), (GHOST0, OBS1),
             (GHOST1, OBS0), (GHOST1, OBS1)]
    variableDomainsDict = {}

    "*** YOUR CODE HERE ***"
    for variable in variables:
        variableDomainsDict[variable] = []
    maxValue = X_RANGE + Y_RANGE + MAX_NOISE
    ...
    for i in range(0, X_RANGE):
        for j in range(0, Y_RANGE):
            position = (i, j)
            variableDomainsDict[PAC].append(position)
            variableDomainsDict[GHOST0].append(position)
            variableDomainsDict[GHOST1].append(position)
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for Variable in eliminationOrder:
            ...
            currentFactorsList.append(joinedFactor)
        fullJoinOnQueryVariable = joinFactors(currentFactorsList)
        result = normalize(fullJoinOnQueryVariable)
        return result
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in positions:
            observationProb = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
            newDistribution[position] = self.beliefs[position] * observationProb
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPosition in self.allPositions:
            posDistribution = self.getPositionDistribution(gameState, oldPosition)
            for newPosition in self.allPositions:
                newDistribution[newPosition] += posDistribution[newPosition] * self.beliefs[oldPosition]
        self.beliefs = newDistribution
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        ParticlePerState = int(self.numParticles / len(self.legalPositions))
        for position in self.legalPositions:
            for _ in range(0, ParticlePerState):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in oldParticles:
            distro = self.getPositionDistribution(gameState, particle)
            successor = distro.sample()
            self.particles.append(successor)
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        states = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(states)
        ParticlePerState = int(self.numParticles / len(states))
        remainder = self.numParticles % len(states)
        for state in states:
            for _ in range(0, ParticlePerState):
                self.particles.append(state)
        self.particles += states[:remainder]
        "*** END YOUR CODE HERE ***"
